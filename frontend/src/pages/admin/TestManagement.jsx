    ...
    <div className="container" style={{ padding: '2rem' }}>
      ...
      <div className="card" style={{ padding: '1.5rem', border: '1px solid #e2e8f0', borderRadius: '8px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
          <h3 style={{ color: '#2d3748', margin: 0 }}>Question Bank Editor</h3>
          <button className="btn btn-danger" onClick={handleResetOriginal}>Reset to Original JSON</button>
        </div>

        {groupedQuestions.map((group, idx) => (
          <div key={idx} style={{ marginBottom: '2rem' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', backgroundColor: '#f7fafc', padding: '0.8rem', borderRadius: '4px', borderLeft: '4px solid #3182ce' }}>
              <h4 style={{ margin: 0, color: '#2b6cb0' }}>{group.section}</h4>
              <span style={{ fontSize: '0.9rem', color: '#718096' }}>{group.items.length} Questions</span>
            </div>

            <div style={{ padding: '1rem' }}>
              {group.items.map(q => (
                <div key={q._id} style={{ padding: '1rem', border: '1px solid #e2e8f0', borderRadius: '8px', marginBottom: '1rem' }}>
                  {editingId === q._id ? (
                    <div>
                      <div className="form-group">
                        <label>Question Number</label>
                        <input type="number" className="form-control" value={editForm.questionNumber} onChange={e => setEditForm({...editForm, questionNumber: e.target.value})} style={{ width: '80px', marginBottom: '0.5rem' }}/>
                        <label>Question Text</label>
                        <textarea className="form-control" value={editForm.questionText} onChange={e => setEditForm({...editForm, questionText: e.target.value})} rows="2" style={{ marginBottom: '0.5rem' }}/>
                      </div>
                      ...
                      <div className="form-group" style={{ marginBottom: '1rem' }}>
                        ...
                        <select className="form-control" value={editForm.correctAnswer} onChange={e => setEditForm({...editForm, correctAnswer: Number(e.target.value)})} style={{ width: '120px' }}>
                      ...
                      <div style={{ display: 'flex', gap: '1rem' }}>
                        <button className="btn btn-primary" onClick={() => saveEdit(q._id)}>Save</button>
                        <button className="btn" style={{ backgroundColor: '#e2e8f0' }} onClick={cancelEdit}>Cancel</button>
                      </div>
                    </div>
                  ) : (
                    <div>
                      <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                        <strong style={{ fontSize: '1.1rem' }}>Q{q.questionNumber}. {q.questionText}</strong>
                        <div>
                          <button className="btn btn-primary" style={{ padding: '0.3rem 0.6rem', fontSize: '0.8rem', marginRight: '0.5rem' }} onClick={() => startEdit(q)}>Edit</button>
                          <button className="btn btn-danger" style={{ padding: '0.3rem 0.6rem', fontSize: '0.8rem' }} onClick={() => deleteQuestion(q._id)}>Delete</button>
                        </div>
                      </div>
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.5rem', marginTop: '1rem', color: '#4a5568' }}>
                        {q.options.map((opt, oIdx) => (
                          <div key={oIdx} style={{ padding: '0.5rem', backgroundColor: q.correctAnswer === oIdx ? '#c6f6d5' : '#edf2f7', borderRadius: '4px', border: q.correctAnswer === oIdx ? '1px solid #48bb78' : 'none' }}>
                            {String.fromCharCode(65 + oIdx)}. {opt} {q.correctAnswer === oIdx && ' (Correct)'}
                          </div>
                        ))}
                      </div>
                    </div>
              ...
              <button className="btn btn-primary" style={{ marginTop: '1rem' }} onClick={() => addQuestion(group.section)}>+ Add Question to {group.section}</button>
            </div>
          </div>
        ))}
      </div>
