...
const Test = () => {
  ...
  useEffect(() => {
    ...
    const initTest = async () => {
      try {
        const { data: qData } = await api.get('/rank-test/questions');
        setQuestions(qData);
        
        const { data: timeData } = await api.post('/rank-test/start');
  ...
  useEffect(() => {
    ...
    const autoSave = setInterval(() => {
      if (Object.keys(answers).length > 0) {
        api.post('/rank-test/save', { answers: Object.values(answers) }).catch(console.error);
      }
    }, 30000);
  ...
  const submitTest = async () => {
    try {
      await api.post('/rank-test/submit', { answersMap: answers });
